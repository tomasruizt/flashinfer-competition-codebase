...
from solution.triton.kernel import kernel_fi_baseline, kernel_fla_recurrent
...
def make_bench_fn(kernel_fn):
    """Create an NVBench benchmark function for the given kernel."""

    def bench_fn(state: bench.State):
...
def main():
    ...
    algos = ALGOS if args.algo == "all" else {args.algo: ALGOS[args.algo]}

    for name, kernel_fn in algos.items():
        print(f"\n{'=' * 60}")
        print(f"  NVBench: {name}")
        print(f"{'=' * 60}")
        bench.register(make_bench_fn(kernel_fn))
